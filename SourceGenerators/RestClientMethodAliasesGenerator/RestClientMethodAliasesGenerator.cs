...
public class RestClientMethodAliasesGenerator : IIncrementalGenerator
{
                                     ...
                                     IReadOnlyList<string> documentation)
    {
        ...
        writer.WriteTypeDeclaration(typeSymbol);

        writer.Write("{");
        writer.Indent++;

        WriteMethodDefinition(writer, typeSymbol, interfaceToImplementSymbol, methodSymbol, parameters, methodName, ref cast, hides, documentation, modifiers, castType, out var returnType);
        ...
        else
        {
            ...
            WriteMethodBody(typeSymbol, interfaceToImplementSymbol, methodSymbol, parameters, writer, cast, castType, clientName);
    ...
    private static void WriteMethodDefinition(IndentedTextWriter writer,
                                              INamedTypeSymbol typeSymbol,
                                              INamedTypeSymbol? interfaceToImplementSymbol,
                                              ...
                                              out ITypeSymbol returnType)
    {
        ...
        {
            var count = documentation.Count;
            for (int i = 0; i < count; i++)
                writer.WriteLine(documentation[i]);
        }
        ...
        writer.Write(returnType.ToDisplayString());
        writer.Write(' ');

        if (explicitImplementation)
        {
            writer.Write(interfaceToImplementSymbol!.ToDisplayString());
            writer.Write('.');
        }
        ...
        for (int i = 0; i < maxIndex; i++)
        {
            ...
            if (!parameter.Alias)
            {
                writer.Write(parameter.Parameter!.ToDefinitionString(explicitImplementation));
                writer.Write(", ");
            }
        }
        ...
        if (!lastParameter.Alias)
            writer.Write(lastParameter.Parameter!.ToDefinitionString(explicitImplementation));
                                        ...
                                        string clientName)
    {
        ...
        {
            writer.Write('(');
            writer.Write((castType ?? interfaceToImplementSymbol ?? typeSymbol).ToDisplayString());
            writer.Write(')');
        ...
        writer.Write(parameters[maxIndex].Name);
    ...
    private static string[]? GetModifiers(AttributeData generateAliasAttributeData)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (argument.Key == "Modifiers")
                return [.. argument.Value.Values.Select(v => (string)v.Value!)];
        }
    ...
    private static IReadOnlyList<string> GetDocumentation(IMethodSymbol methodSymbol, ParameterInfo[] parameters)
    {
        return methodSymbol.GetXmlCommentLines(node =>
        {
