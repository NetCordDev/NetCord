...
public static class ClassHandlerInvocationHelper
{
    ...
    {
        writer.WriteLine("if (handlerMetadata is SingletonClassHandlerMetadata singletonHandlerMetadata)");

        writer.Indent++;
        writer.Write("handler = ((");
        writer.Write(handlerTypeName);
        writer.Write(")instanceFactory(services)).HandleAsync;");
        ...
        writer.WriteLine("var nonSingletonHandlerMetadata = (NonSingletonClassHandlerMetadata)handlerMetadata;");

        writer.WriteLine("var flags = nonSingletonHandlerMetadata.Flags;");
