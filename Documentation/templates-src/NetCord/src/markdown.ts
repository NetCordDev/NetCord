...
async function renderMermaid() {
  ...
  const processedDiagrams = document.querySelectorAll<HTMLElement>(
    "pre.mermaid[data-mermaid]",
  );
  ...
  mermaid.initialize(
    Object.assign({ startOnLoad: false, theme }, mermaidOptions),
  );
...
function renderWordBreaks() {
  document
    .querySelectorAll<HTMLElement>(
      "article h1,h2,h3,h4,h5,h6,.xref,.text-break",
    )
    .forEach((e) => {
...
async function renderClickableImage() {
  ...
  const imageLinks = Array.from(
    document.querySelectorAll<HTMLImageElement>("article a img[src]"),
  );

  document
    .querySelectorAll<HTMLImageElement>("article img[src]")
    .forEach((img) => {
      ...
      function shouldMakeClickable(): boolean {
        ...
        return (
          img.naturalWidth > MIN_CLICKABLE_IMAGE_SIZE &&
...
function renderLinks() {
  ...
  document
    .querySelectorAll<HTMLAnchorElement>("article a[href]")
    .forEach((a) => {
      if (
...
function renderCodeCopy() {
  document.querySelectorAll<HTMLElement>("pre>code").forEach((code) => {
    ...
    function renderCore() {
      const dom = copied
        ? html`<a class="btn border-0 link-success code-action"
            ><i class="bi bi-check-lg"></i
          ></a>`
        : html`<a
            class="btn border-0 code-action"
            title="${loc("copy")}"
            href="#"
            @click=${copy}
...
function renderTabs() {
  ...
  function updateTabStyle() {
    document
      .querySelectorAll("div.tabGroup>ul")
      .forEach((e) => e.classList.add("nav", "nav-tabs"));
    document
      .querySelectorAll("div.tabGroup>ul>li")
      .forEach((e) => e.classList.add("nav-item"));
    document
      .querySelectorAll("div.tabGroup>ul>li>a")
      .forEach((e) => e.classList.add("nav-link"));
    document
      .querySelectorAll("div.tabGroup>section")
      .forEach((e) => e.classList.add("card"));
  }
