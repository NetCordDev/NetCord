...
export async function enableSearch() {
  ...
  function renderSearchResults(hits: SearchHit[], page: number) {
    ...
    render(renderPage(page), document.getElementById("search-results"));
    ...
    function renderPage(page: number): TemplateResult {
      ...
      const items = html` <div class="search-list">
          ${loc("searchResultsCount", { count: hits.length.toString(), query })}
        </div>
        <div class="sr-items">
          ${curHits.map((hit) => {
            const currentUrl = window.location.href;
            const itemRawHref = relativeUrlToAbsoluteUrl(
              currentUrl,
              relHref + hit.href,
            );
    ...
    function renderPagination() {
      ...
      const pages = Array.from(new Array(endPage - startPage).keys()).map(
        (i) => i + startPage,
      );
      ...
      return html` <nav>
        <ul class="pagination">
          <li class="page-item">
            <a
              class="page-link ${classMap({ disabled: page <= 0 })}"
              href="#"
              aria-label="Previous"
          ...
          ${pages.map(
            (i) =>
              html` <li class="page-item">
                <a
                  class="page-link ${classMap({ active: page === i })}"
                  href="#"
                  @click="${() => gotoPage(i)}"
                  >${i + 1}</a
                >
