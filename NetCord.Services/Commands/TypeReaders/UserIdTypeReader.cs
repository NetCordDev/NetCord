...
public class UserIdTypeReader<TContext> : CommandTypeParser<TContext> where TContext : ICommandContext
{
    public override ValueTask<CommandTypeParserResult> ParseAsync(ReadOnlyMemory<char> input, TContext context, CommandParameter<TContext> parameter, CommandServiceConfiguration<TContext> configuration, IServiceProvider? serviceProvider)
    {
        ...
        if (guild is null)
        {
            if (context.Message.Channel is DMChannel dMChannel)
            {
                ...
                if (Mention.TryParseUser(span, out var id))
                    return new(CommandTypeParserResult.Success(new UserId(id, users.GetValueOrDefault(id))));
                    ...
                    if (ushort.TryParse(span[^4..], NumberStyles.None, CultureInfo.InvariantCulture, out var discriminator))
                    {
                        ...
                        foreach (var user in users.Values)
                        {
                            if (user.Discriminator == discriminator && username.SequenceEqual(user.Username))
                                return new(CommandTypeParserResult.Success(new UserId(user.Id, user)));
                        }
                    }
                }
                ...
                if (input.Length is <= 32 and >= 2)
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        ...
                        if (span.SequenceEqual(current.Username))
                        {
                            ...
                            return new(CommandTypeParserResult.Success(new UserId(current.Id, current)));
                        }
                    }
                }
        ...
        TooManyFound:
        return new(CommandTypeParserResult.Fail("Too many users found."));
    }
}
