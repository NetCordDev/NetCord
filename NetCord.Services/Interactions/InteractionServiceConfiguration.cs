...
public record InteractionServiceConfiguration<TContext> where TContext : IInteractionContext
{
    public static InteractionServiceConfiguration<TContext> Default { get; } = new();

    private InteractionServiceConfiguration()
    {
    }

    public ImmutableDictionary<Type, InteractionTypeReader<TContext>> TypeReaders { get; init; } = new Dictionary<Type, InteractionTypeReader<TContext>>()
    {
    #region TypeReaders
        // text types
