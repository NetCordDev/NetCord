...
public class TimeSpanTypeReader<TContext> : ComponentInteractionTypeReader<TContext> where TContext : IComponentInteractionContext
{
    ...
    {
        return new(TimeSpanTypeReaderHelper.Read(input.ToString(), configuration.IgnoreCase, configuration.CultureInfo) is { } value ? ComponentInteractionTypeReaderResult.Success(value) : ComponentInteractionTypeReaderResult.ParseFail(parameter.Name));
    }
}
