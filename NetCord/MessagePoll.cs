...
public class MessagePoll(JsonMessagePoll jsonModel, ulong guildId, RestClient client) : IJsonModel<JsonMessagePoll>
{
    ...
    public IReadOnlyList<MessagePollAnswer> Answers = jsonModel.Answers.Select(x => new MessagePollAnswer(x, guildId, client)).ToImmutableList();
    ...
    public DateTimeOffset? ExpireAt => jsonModel.ExpireAt;
