...
public partial class RestClient
{
    ...
    => new(await (await SendRequestAsync(HttpMethod.Post, $"/guilds/{guildId}/stickers", sticker.Build(), properties).ConfigureAwait(false)).ToObjectAsync(JsonModels.JsonSticker.JsonStickerSerializerContext.WithOptions.JsonSticker).ConfigureAwait(false), this);
    ...
    public async Task<GuildSticker> ModifyGuildStickerAsync(ulong guildId, ulong stickerId, Action<GuildStickerOptions> action, RequestProperties? properties = null)
    {
        ...
        return new(await (await SendRequestAsync(HttpMethod.Patch, $"/guilds/{guildId}/stickers/{stickerId}", new(RateLimits.RouteParameter.ModifyGuildSticker), new JsonContent<GuildStickerOptions>(guildStickerOptions, GuildStickerOptions.GuildStickerOptionsSerializerContext.WithOptions.GuildStickerOptions), properties).ConfigureAwait(false)).ToObjectAsync(JsonModels.JsonSticker.JsonStickerSerializerContext.WithOptions.JsonSticker).ConfigureAwait(false), this);
    }
