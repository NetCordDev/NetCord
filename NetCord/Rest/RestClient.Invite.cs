...
public partial class RestClient
{
    ...
    public async Task<RestGuildInvite> DeleteGuildInviteAsync(string inviteCode, RequestProperties? properties = null)
        => new(await (await SendRequestAsync(HttpMethod.Delete, $"/invites/{inviteCode}", new RateLimits.Route(RateLimits.RouteParameter.DeleteGuildInvite), properties).ConfigureAwait(false)).ToObjectAsync(JsonModels.JsonRestGuildInvite.JsonRestGuildInviteSerializerContext.WithOptions.JsonRestGuildInvite).ConfigureAwait(false), this);
}
