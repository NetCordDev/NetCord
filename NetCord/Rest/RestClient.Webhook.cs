...
public partial class RestClient
{
    public async Task<Webhook> CreateWebhookAsync(ulong channelId, WebhookProperties webhookProperties, RequestProperties? properties = null)
    {
        using (HttpContent content = new JsonContent<WebhookProperties>(webhookProperties, WebhookProperties.WebhookPropertiesSerializerContext.WithOptions.WebhookProperties))
            return Webhook.CreateFromJson(await (await SendRequestAsync(HttpMethod.Post, content, $"/channels/{channelId}/webhooks", null, new(channelId), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookSerializerContext.WithOptions.JsonWebhook).ConfigureAwait(false), this);
    }
    ...
    public async Task<IReadOnlyDictionary<ulong, Webhook>> GetChannelWebhooksAsync(ulong channelId, RequestProperties? properties = null)
        => (await (await SendRequestAsync(HttpMethod.Get, $"/channels/{channelId}/webhooks", null, new(channelId), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookArraySerializerContext.WithOptions.JsonWebhookArray).ConfigureAwait(false)).ToDictionary(w => w.Id, w => Webhook.CreateFromJson(w, this));
    ...
    public async Task<IReadOnlyDictionary<ulong, Webhook>> GetGuildWebhooksAsync(ulong guildId, RequestProperties? properties = null)
        => (await (await SendRequestAsync(HttpMethod.Get, $"/guilds/{guildId}/webhooks", null, new(guildId), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookArraySerializerContext.WithOptions.JsonWebhookArray).ConfigureAwait(false)).ToDictionary(w => w.Id, w => Webhook.CreateFromJson(w, this));

    public async Task<Webhook> GetWebhookAsync(ulong webhookId, RequestProperties? properties = null)
        => Webhook.CreateFromJson(await (await SendRequestAsync(HttpMethod.Get, $"/webhooks/{webhookId}", null, new(webhookId), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookSerializerContext.WithOptions.JsonWebhook).ConfigureAwait(false), this);

    public async Task<Webhook> GetWebhookWithTokenAsync(ulong webhookId, string webhookToken, RequestProperties? properties = null)
        => Webhook.CreateFromJson(await (await SendRequestAsync(HttpMethod.Get, $"/webhooks/{webhookId}/{webhookToken}", null, new(webhookId, webhookToken), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookSerializerContext.WithOptions.JsonWebhook).ConfigureAwait(false), this);
    ...
    public async Task<Webhook> ModifyWebhookAsync(ulong webhookId, Action<WebhookOptions> action, RequestProperties? properties = null)
    {
        ...
        using (HttpContent content = new JsonContent<WebhookOptions>(webhookOptions, WebhookOptions.WebhookOptionsSerializerContext.WithOptions.WebhookOptions))
            return Webhook.CreateFromJson(await (await SendRequestAsync(HttpMethod.Patch, content, $"/webhooks/{webhookId}", null, new(webhookId), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookSerializerContext.WithOptions.JsonWebhook).ConfigureAwait(false), this);
    }
    ...
    public async Task<Webhook> ModifyWebhookWithTokenAsync(ulong webhookId, string webhookToken, Action<WebhookOptions> action, RequestProperties? properties = null)
    {
        ...
        action(webhookOptions);
        using (HttpContent content = new JsonContent<WebhookOptions>(webhookOptions, WebhookOptions.WebhookOptionsSerializerContext.WithOptions.WebhookOptions))
            return Webhook.CreateFromJson(await (await SendRequestAsync(HttpMethod.Patch, content, $"/webhooks/{webhookId}/{webhookToken}", null, new(webhookId, webhookToken), properties).ConfigureAwait(false)).ToObjectAsync(JsonWebhook.JsonWebhookSerializerContext.WithOptions.JsonWebhook).ConfigureAwait(false), this);
    }
    ...
    public async Task<RestMessage> GetWebhookMessageAsync(ulong webhookId, string webhookToken, ulong messageId, RequestProperties? properties = null)
        => new(await (await SendRequestAsync(HttpMethod.Get, $"/webhooks/{webhookId}/{webhookToken}/messages/{messageId}", null, new(webhookId, webhookToken), properties).ConfigureAwait(false)).ToObjectAsync(JsonMessage.JsonMessageSerializerContext.WithOptions.JsonMessage).ConfigureAwait(false), this);
