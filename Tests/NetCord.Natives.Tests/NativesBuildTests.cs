...
public class NativesBuildTests
{
    ...
    public void NativeAotStaticLinking(string libName)
    {
        try
        {
            ...
            var nativesDir = assembly.GetCustomAttribute<AssemblyMetadataAttribute>()?.Value;
            ...
            buildProcess.StartInfo.ArgumentList.Add($"-p:NetCordDirectPInvoke=\"{libName}\"");

            TestContext.WriteLine($"Building Native AoT app: 'dotnet {buildProcess.StartInfo.ArgumentList.Aggregate((a, b) => $"{a} {b}")}'");

            buildProcess.StartInfo.WorkingDirectory = Path.Combine(AppContext.BaseDirectory, "Assets", "NativeAotApp");
            ...
            aotProcess.StartInfo.FileName = Path.Combine(AppContext.BaseDirectory, "Assets", "NativeAotApp", 
                "bin", "release", RuntimeInformation.RuntimeIdentifier, "publish", "NativeAotApp.exe");
