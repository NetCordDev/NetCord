...
public abstract class ServiceTests<TTester> where TTester : ServiceTester, new()
{
    protected readonly TTester _tester = new();
    ...
    public async ValueTask ParameterParsing()
    {
        ...
        (string, decimal?)[] decimals = [
            ...
            ("3.14159e1", 31.4159m),
            ("-2.71828E2", -271.828m),
            ("1.0e-10", 0.0000000001m),
            ("-1.234e-4", -0.0001234m),
            ("5e-1", 0.5m),
            ("1.23456e4", 12345.6m),
            ("-9.87654e-3", -0.00987654m),
    ...
    public async ValueTask OptionalParameterValues()
    {
        ...
        if (_tester.SupportsReadOnlyMemoryChar)
        ...
        {
            await _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T>(default), ([Optional] T value) => Body.Data(value)).ConfigureAwait(false);

            await _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T?>(null), ([Optional] T? value) => Body.Data(value)).ConfigureAwait(false);
        }

        ValueTask OnlyOptionalAttributeReferenceType<T>() where T : class
        {
            return _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T?>(null), ([Optional] T? value) => Body.Data(value));
        }

        ValueTask OptionalWithCustomDefaultValueValueType<T>(T defaultValue, Delegate handler) where T : struct
        {
            return _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch(defaultValue), handler);
        }
        ...
        {
            await _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T>(default), (T value = default) => Body.Data(value)).ConfigureAwait(false);

            await _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T?>(null), (T? value = null) => Body.Data(value)).ConfigureAwait(false);
        }

        ValueTask OptionalWithDefaultValueReferenceType<T>() where T : class
        {
            return _tester.ExecuteNoArgumentsAsync("test", ResultHandler.DataMatch<T?>(null), (T? value = null) => Body.Data(value));
        }
    }
}
