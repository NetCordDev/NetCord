...
public class AdministrativeInteractions : BaseInteractionModule<ButtonInteractionContext>
{
    ...
    public async Task UnbanAsync(ulong userId)
    {
        ...
        await Context.Interaction.SendResponseAsync(InteractionCallback.Message(new() { Content = $"**Ban cancelled by {Context.User}**", AllowedMentions = AllowedMentionsProperties.None, Components = Enumerable.Empty<ComponentProperties>() }));
    }
    ...
    public async Task UnmuteAsync(ulong userId)
    {
        ...
        await Context.Interaction.SendResponseAsync(InteractionCallback.Message(new() { Content = $"**Mute cancelled by {Context.User}**", AllowedMentions = AllowedMentionsProperties.None, Components = Enumerable.Empty<ComponentProperties>() }));
    }
}
